...
CPF::operator uint64_t() const {
    std::cout << "Convertendo cpf\n";
    return static_cast<uint64_t>(this->numero);
}

CPF::operator std::string() const { return std::to_string(this->numero); }
...
unsigned long CPF::operator()(short int inicio, short int tam) const {
    ...
    unsigned long retorno{
        (unsigned long)(this->numero / (std::pow(10, 10 - inicio - tam + 1)))};
